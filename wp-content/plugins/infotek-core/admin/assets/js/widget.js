 ;(function ($) {
    ...
    $(document).ready(function () {
        /***********************************
         infotek Custom widget js
         ************************************/
        $(document).on('widget-updated',function (e,widget) {
            var widget_id = $(widget).attr('id');
            if (widget_id.indexOf('infotek_about_us_widget') != 1){
                prefetch();
            }
        });

        $('body').off('click','.infotek_flogo_uploader');

        $('body').on('click','.infotek_flogo_uploader',function (e) {
            var el = this;
            ...
            file_uploder_frame.on('insert',function () {
                ...
                var img_prev_container = $(el).siblings('.infotek-logo-preview');
                ...
                $(el).siblings().children('.infotek_logo_id').val(selected_ids.join(","));
                $(el).siblings().children('.infotek_logo_id').trigger('change');
            ...
            file_uploder_frame.on('open',function () {
                ...
                console.log($(el).siblings().children('.infotek_logo_id').val());
                var atts = $(el).siblings().children('.infotek_logo_id').val().split(",");
        ...
        function prefetch () {
            $('.infotek_logo_id').each(function () {
                var attid = $(this).val();
                var container = $(this).parent().siblings('.infotek-logo-preview');
                container.html('');

                if ( attid ){
                    $(this).parent().parent().find('.infotek_flogo_uploader').val('Change Image');
