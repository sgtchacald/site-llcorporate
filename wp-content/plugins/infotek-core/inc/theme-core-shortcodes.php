...
if (!class_exists('Infotek_Core_shortcodes')) {

    class Infotek_Core_shortcodes
    {
        ...
        {
            //social post share
            add_shortcode('infotek_post_share', array($this, 'post_share'));
            //social icon
            add_shortcode('infotek_social_icon_wrap', array(__CLASS__, 'social_icon_wrap'));
            add_shortcode('infotek_social_icon', array(__CLASS__, 'social_icons'));
            //top menu
            add_shortcode('infotek_top_menu_wrap', array(__CLASS__, 'top_menu_wrap'));
            add_shortcode('infotek_top_menu', array(__CLASS__, 'top_menu'));
            //top menu
            add_shortcode('infotek_top_menu_wrap_02', array(__CLASS__, 'top_menu_wrap_02'));
            add_shortcode('infotek_top_menu_02', array(__CLASS__, 'top_menu_02'));
            //info_item
            add_shortcode('infotek_info_item_wrap', array(__CLASS__, 'info_item_wrap'));
            add_shortcode('infotek_info_link', array(__CLASS__, 'info_link'));
            add_shortcode('infotek_info_inline_text', array(__CLASS__, 'info_inline_text'));
        ...
        public static function post_share($atts, $content = null)
        {
            ...
            if (is_singular() || is_home()) {
                ...
                $infotek_url = urlencode_deep(get_permalink());
                //get current page title
                $infotek_title = str_replace(' ', '%20', get_the_title($post->ID));
                //get post thumbnail for pinterest
                $infotek_thumbnail = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'full');
                $infotek_thumbnail = !empty($infotek_thumbnail) ? $infotek_thumbnail[0] : '';

                //all social share link generate
                $facebook_share_link = 'https://www.facebook.com/sharer/sharer.php?u=' . $infotek_url;
                $twitter_share_link = 'https://twitter.com/intent/tweet?text=' . $infotek_title . '&amp;url=' . $infotek_url . '&amp;via=' . get_bloginfo('url');
                $linkedin_share_link = 'https://www.linkedin.com/shareArticle?mini=true&url=' . $infotek_url . '&amp;title=' . $infotek_title;
                $instagram_share_link = 'https://www.instagram.com/?url=' . $infotek_url;
    ...
    if (class_exists('Infotek_Core_shortcodes')) {
        Infotek_Core_shortcodes::getInstance();
    }
