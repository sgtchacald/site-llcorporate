...
class Infotek_Tags_Widget extends WP_Widget
{
    ...
    public function __construct()
    {
        parent::__construct(
            'infotek_tags',
            esc_html__('Infotek: Tags', 'infotek-core'),
...
//            'type' => $post_type,
        ...
        if (!empty($tags)):?>
            ...
            esc_html__(' Oops, there are no tags.', 'infotek-core')
            ?>
        <?php endif; ?>
        </div>
        <?php
    ...
    public function form($instance)
    {
        ...
        if (!empty($instance) && $instance['title']) {
            ...
            $title = esc_html__('Tags', 'infotek-core');

        }
        $order = !empty($instance) && $instance['order'] ? $instance['order'] : 'ASC';
        $orderby = !empty($instance) && $instance['orderby'] ? $instance['orderby'] : 'ID';
        $post_tags = array(
            'post_tag' => esc_html__('Blog', 'infotek-core'),
            'packages-tag' => esc_html__('Packages', 'infotek-core'),
            'project-tag' => esc_html__('Project', 'infotek-core'),
            'course-tag' => esc_html__('Courses', 'infotek-core'),
            'service-tag'=> esc_html__('Service', 'infotek-core')
        );
        $order_arr = array(
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('post_tags')) ?>"><?php esc_html_e('Post Tags', 'infotek-core'); ?></label>
...
if (!function_exists('Infotek_Tags_Widget')) {
    function Infotek_Tags_Widget()
    {
        register_widget('Infotek_Tags_Widget');
    }

    add_action('widgets_init', 'Infotek_Tags_Widget');
}